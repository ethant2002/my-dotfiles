...
choco install hackfont-nerd-font -y --ignore-checksums
...
if (Test-Path $terminalSettingsPath) {
    ...
    # Set history size
    $settings.profiles.defaults | Add-Member -MemberType NoteProperty -Name "historySize" -Value 12000 -Force
    
    # Set opacity (no acrylic)
    $settings.profiles.defaults | Add-Member -MemberType NoteProperty -Name "opacity" -Value 95 -Force
    $settings.profiles.defaults | Add-Member -MemberType NoteProperty -Name "useAcrylic" -Value $false -Force
    
    # Set scrollbar state
    $settings.profiles.defaults | Add-Member -MemberType NoteProperty -Name "scrollbarState" -Value "visible" -Force
    
    # Save updated settings
    $settings | ConvertTo-Json -Depth 100 | Set-Content $terminalSettingsPath -Force
    
    Write-Host "Windows Terminal configured with Hack Nerd Font and xcad color scheme!" -ForegroundColor Green
} else {
...
Write-Host "  - Hack Nerd Font" -ForegroundColor White
Write-Host ""
